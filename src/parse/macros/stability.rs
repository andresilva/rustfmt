...
fn parse_stability_mod_inner<'a>(
    ...
    while parser.token.kind != TokenKind::Eof {
        match parser.token.kind {
            _ if parser.token.kind.open_delim().is_some() => depth += 1,
            _ if parser.token.kind.close_delim().is_some() => depth = depth.saturating_sub(1),
...
fn parse_stability_scope_inner<'a>(
    ...
    while parser.token.kind != TokenKind::CloseBrace && parser.token.kind != TokenKind::Eof {
        let before_span = parser.token.span;
        let had_errors_before = parser.psess.dcx().has_errors().is_some();
