...
fn try_parse_expr(parser: &mut Parser<'_>) -> Option<Box<ast::Expr>> {
...
fn try_parse_pat(parser: &mut Parser<'_>) -> Option<Box<ast::Pat>> {
    match parser.parse_pat_no_top_alt(None, None) {
        Ok(pat) if parser.psess.dcx().has_errors().is_none() => Some(Box::new(pat)),
        Ok(_) => {
            parser.psess.dcx().reset_err_count();
            None
        }
...
enum KeywordArmResult {
    ...
    Parsed { span: Span, body: Box<ast::Expr> },
...
pub(crate) struct SelectBranch {
    ...
    pub span: Span,
    pub pattern: Box<ast::Pat>,
    pub future_expr: Box<ast::Expr>,
    /// Optional diverging expression for let-else patterns (e.g., `else break`).
    pub else_diverge: Option<Box<ast::Expr>>,
...
pub(crate) enum SelectLoopItem {
    OnStart { span: Span, body: Box<ast::Expr> },
    OnStopped { span: Span, body: Box<ast::Expr> },
    Branch(SelectBranch),
    OnEnd { span: Span, body: Box<ast::Expr> },
}
